...
class Thrive_Dash_List_Connection_AWeber extends Thrive_Dash_List_Connection_Abstract {
	...
	protected function get_middleman_authorize_url() {
		...
		// Return the authorization URL for simple redirect (like OAuth 1.0)
		if ( empty( $result['data']['authorization_url'] ) ) {
			return __( 'Authorization URL not received from API', 'thrive-dash' );
		}
	...
	protected function add_subscriber_oauth2( $list_identifier, $arguments ) {
		...
		try {
			...
			// Handle error
			$error_message = ! empty( $response_data['error']['message'] ) ? $response_data['error']['message'] : __( 'Failed to add subscriber', 'thrive-dash' );
	...
	private function add_subscriber_middleman( $list_identifier, $arguments ) {
		...
		if ( empty( $result['success'] ) ) {
			return $result['error'] ?? __( 'Failed to add subscriber', 'thrive-dash' );
		}
