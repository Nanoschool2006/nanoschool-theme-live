...
class Thrive_Dash_Api_AWeber_OAuth2_Middleman {
		...
		if ( ! $response['success'] ) {
			...
			switch ( $error_code ) {
				case 'connection_timeout':
					$user_error = 'Connection timed out while contacting Thrive API server. Please check your internet connection and try again.';
					break;
				case 'ssl_verification_failed':
					$user_error = 'SSL certificate verification failed. Your server may have outdated SSL certificates. Please contact your hosting provider.';
					break;
				case 'network_error':
					$user_error = 'Could not connect to Thrive API server. Please check your internet connection and ensure your server can make outbound HTTPS requests.';
					break;
				case 'invalid_site_id':
					$user_error = 'Your site URL is not valid. Please check your WordPress site URL settings.';
					break;
				default:
					$user_error = sprintf( 'API returned error: %s. Please try again or contact support if the problem persists.', esc_html( $error_msg ) );
					break;
			}
	...
	public function proxy_request( $method, $endpoint, $data = array() ) {
		...
		$url      = $base_url . '?site_id=' . rawurlencode( $this->site_id );
