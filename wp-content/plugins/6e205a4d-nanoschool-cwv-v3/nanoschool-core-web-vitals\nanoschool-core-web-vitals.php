...
class Nanoschool_CWV_Optimizer {
    ...
    public function __construct() {
        ...
        // 6. Optimize LCP Images (Fetch Priority)
        add_filter('wp_get_attachment_image_attributes', array($this, 'optimize_lcp_images'), 10, 3);
        add_filter('the_content', array($this, 'optimize_content_lcp_images'));
    }
    ...
    /**
     * Add high fetch priority to LCP images (Logo, key banners)
     */
    public function optimize_lcp_images($attrs, $attachment, $size) {
        $lcp_images = array('NSTC-Logo-2-removebg-preview', 'chip', 'research', 'microscope');
        foreach ($lcp_images as $slug) {
            if (isset($attrs['src']) && strpos($attrs['src'], $slug) !== false) {
                $attrs['fetchpriority'] = 'high';
                $attrs['loading'] = 'eager'; // Ensure it's not lazy loaded
            }
        }
        return $attrs;
    }
    ...
    /**
     * Content filter to add fetchpriority manually for hardcoded images
     */
    public function optimize_content_lcp_images($content) {
        if (is_front_page()) {
